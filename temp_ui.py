...
Dua card yang dulu duduk di halaman utama ``app.py`` dipindah ke halaman ini
(permintaan user 2026-09-16: *"pindah ke page baru TEMP"*) supaya halaman utama
tinggal **🏆 Scan Best Pool Meteora** + **🧾 Log Aktivitas**. Logika render,
session key, widget key dan tooltip dipindah **apa adanya** dari ``app.py``;
yang berubah hanya penempatannya + auto-refresh yang kini per halaman:

- :func:`render_auto_refresh` — toggle 🔄 Auto-refresh ±60 dtk + fragment yang
  membandingkan ``updated_at`` ``holder_status.json`` (snapshot cron) dan
  ``st.rerun()`` saat ada snapshot baru. Dulu satu-satunya di halaman utama;
  sekarang tiap halaman punya sendiri, karena halaman utama tidak lagi membaca
  store watchlist/holder (aturan auto-refresh 2026-09-08 tetap sama);
- :func:`render_lp_watchlist_section` — card 🌊 Watchlist Meteora (baris + form
  ➕ + 🔄 Scan sekarang + grafik perubahan dust + alert Telegram);
- :func:`render_solana_holder_scan` — section 🛰 Scan Holder Solana (form CA →
  scan Helius FULL + bar chart kedalaman holder);
- :func:`render_temp_page` — halaman penuh: ketiganya berurutan.

Modul ini **tidak mengimpor ``app.py``** — ``app.py`` memanggil
``st.set_page_config`` + ``page_router.apply()`` di top level, jadi mengimpornya
dari halaman lain akan menabrak (set_page_config dua kali / router berjalan
ulang). Sebaliknya, halaman utama juga tidak mengimpor modul ini: card yang
pindah tidak dirender di sana lagi.
"""
...
# Fungsi yang bisa di-patch test dipanggil LEWAT MODULE-NYA (``wl.add_to_watchlist``,
# ``hs.publish_holder_status``, ``hh.ingest_many``, …), bukan lewat
# ``from x import y``: halaman ``app.py`` lama dieksekusi ulang tiap run sehingga
# from-import selalu menangkap mock yang sedang aktif, sedangkan modul
# ``temp_ui`` hanya diimpor sekali — binding from-import akan membekukan fungsi
# asli dan seluruh ``mock.patch("watchlist.add_to_watchlist")`` di test menjadi
# no-op (gejala: tombol ➕ / 🔄 Scan sekarang "tidak memanggil siapa pun").
# Konstanta dan helper murni tampilan tetap diimpor langsung.
import alert_settings
import holder_analysis
import holder_history as hh
import holder_status as hs
import helius_holders
import lp_watchlist
import meteora_screener
import meteora_watchlist
...
from dashboard_components import (_alert_toggle_button, _ca_error, _compact,
                                  ...
                                  _wib, _depth_tables_html, card_head_html,
                                  hover_title_html, SOLANA_CA_RE,
                                  load_dashboard_data)
# ---------------------------------------------------------------------------
...
def render_auto_refresh() -> None:
    """Toggle 🔄 auto-refresh + fragment-nya — hanya berlaku di halaman ini (2026-09-16: card yang membaca store-nya pindah ke 📦 TEMP)."""
...
def _render_lp_row(row: dict) -> None:
    ...
    if cols[9].button("📋", key=f"lp-move-{mint}",
        ...
        wl.set_watchlist_source(mint, "manual", background=True)
        st.rerun()
    if cols[10].button("✕", key=f"lp-remove-{mint}",
                       help="Hapus dari Watchlist Meteora",
                       use_container_width=True):
        wl.remove_from_watchlist(mint, background=True)
        st.rerun()
...
def _render_lp_card(lp_watch: dict, status_tokens: dict,
                    history_store: dict, *, full_watchlist: dict,
    ...
    ``full_watchlist`` / ``prev_status`` = watchlist penuh (termasuk token
    non-LP) + snapshot ``holder_status`` sebelum scan: tombol 🔄 Scan sekarang
    mem-publish ulang status gabungan dari keduanya. Di ``app.py`` dulu ini
    global modul (diisi wiring ``load_dashboard_data()`` di bawah file); sejak
    card pindah ke halaman TEMP keduanya jadi argumen eksplisit supaya tidak
    ada lagi state global yang dibaca diam-diam.
    """
    watchlist = dict(full_watchlist or {})
    holder_status = dict(prev_status or {})
    rows = lp_watchlist.lp_card_rows(lp_watch, status_tokens, history_store)
    summary = lp_watchlist.lp_summary(rows)
    # Jumlah token yang notif Telegram-nya dimatikan user (toggle 🔔/🔕 di
    # baris) — ditampilkan sebagai pill di kepala card.
    summary["muted"] = len(alert_settings.mutes_for(lp_watch or {}))
            ...
            with st.form(LP_ADD_FORM, clear_on_submit=True):
                ...
                if st.form_submit_button("🌊 Tambah ke Watchlist Meteora"):
                    ...
                    if ca_error:
                        ...
                        wl.add_to_watchlist(ca, "?", source=LP_SOURCE,
                                         background=True)
        ...
        if st.button("🔄 Scan sekarang Watchlist Meteora", type="primary",
            ...
            for mint, meta in (lp_watch or {}).items():
                try:
                    ...
                    analyses[mint] = holder_analysis.analyze_token(
            ...
            fresh = {mint: item for mint, item in ok.items()
                     if hh.holders_usable(item.get("holders"))}
            ...
            metric_deliveries = []
            try:
                metric_snapshots = meteora_screener.fetch_watchlist_metric_snapshots(lp_watch)
                metric_result = meteora_watchlist.apply_metric_snapshots(
                ...
                if metric_result.get("baseline_changed"):
                    ...
                    wl.save_watchlist(full_watchlist,
                                   "meteora: simpan baseline metrik",
                                   background=True)
                metric_deliveries = meteora_watchlist.send_metric_alerts(
            ...
            published = None
            if fresh:
                hh.ingest_many(fresh, store=history_store, detail=False)
            if fresh or metric_result.get("metrics"):
                published = hs.publish_holder_status(
    ...
    if run:
        ...
        if not ca:
            ...
            with st.status("Mengambil holder dari Helius…",
                ...
                try:
                    result = helius_holders.scan_token_holders(
...
# ---------------------------------------------------------------------------
# Entry point halaman 📦 TEMP (2026-09-16)
# ---------------------------------------------------------------------------
def render_lp_watchlist_section() -> None:
    """🌊 Watchlist Meteora — dibaca dari snapshot cron + store lokal."""
    data = load_dashboard_data()
    watchlist = data.watchlist
    holder_status = data.status
    history_store = data.history
    status_tokens = holder_status.get("tokens") or {}
    lp_watch, _ = lp_watchlist.split_watchlist(watchlist)
    _render_lp_card(lp_watch, status_tokens, history_store,
                    full_watchlist=watchlist, prev_status=holder_status)


def render_solana_holder_scan() -> None:
    """🛰 Scan Holder Solana.

    Divider pembatas antar card sudah dibawa ``_render_helius_holder_scan``
    (sama seperti saat card ini masih di halaman utama), jadi pemanggil cukup
    menaruh section ini setelah card lain.
    """
    _render_helius_holder_scan()


def render_temp_page() -> None:
    """Halaman 📦 TEMP utuh: auto-refresh + Watchlist Meteora + Scan Holder."""
    render_auto_refresh()
    render_lp_watchlist_section()
    render_solana_holder_scan()
