...
# Header peringatan 🚨 STOP DEGEN — teks **verbatim** dari permintaan user
# 2026-09-17 (jangan "diperbaiki" ejaan/tandanya: user menulisnya begitu, dan
# tes mem-pin string-nya). Tiap baris dibungkus emoticon warning di kiri-kanan.
DEGEN_STOP_EMOJI = "🚨"
DEGEN_STOP_LINES = (
    ("STOP DEGEN, GAK BISA", DEGEN_STOP_EMOJI),
...
def render_degen_stop_header() -> None:
    ...
    *"merah menyala berkedip, tulisan besar, ada emoticon warning"*.
    ...
    ``role="alert"`` menandai blok ini sebagai peringatan untuk pembaca layar.
    """
    lines = "".join(
        f'<span class="degen-stop-line">'
        f'<span class="degen-stop-emoji">{icon}</span>'
        f'{html.escape(text)}'
        f'<span class="degen-stop-emoji">{icon}</span>'
        f'</span>'
